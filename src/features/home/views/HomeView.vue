<template>
  <div class="home-view">
      ...
      <div class="clearup-system-container">
        ...
        <div class="third-section-container">
          ...
          <div class="third-section-header" data-section-id="third-header">
            <h2 class="third-section-title">{{ homeContent?.systemTitle || 'The Clear up System™' }}</h2>
            <p class="third-section-description">{{ homeContent?.systemDescription || 'An exclusive done-for-you marketing infrastructure built for predictable growth.' }}</p>
          </div>

          <!-- System Card Container -->
          <div class="system-card-container">
            <div class="system-card" data-section-id="system-card">
              <h3 class="system-card-title">{{ homeContent?.systemCardTitle || 'Done-for-you Clear Up System' }}</h3>
              <template v-if="homeContent?.systemCardText && homeContent.systemCardText.length > 0">
                <p v-for="(text, index) in homeContent.systemCardText" :key="index" class="system-card-text">{{ text }}</p>
              </template>
              <template v-else>
              ...
              <p class="system-card-roi">{{ homeContent?.systemCardROI || 'TURN EVERY $1 INTO $3893 IN REVENUE OUR SYSTEM IS BUILT TO MAXIMIZE ROI , NOT JUST SPEND YOUR BUDGET' }}</p>
            </div>
          </div>
    ...
    <div v-if="!isSectionDisabled('real-results')" class="real-results-section" data-section-id="real-results-section">
      ...
      <!-- Footer Section -->
      <FooterSection v-if="!isSectionDisabled('footer')" />
...
/* Footer styles moved to FooterSection component */
