<template>
  <div class="legal-view">
    ...
    <PageMaintenance 
      v-if="isPageDisabled('cookie-policy-page') && !isAdminUser"
    ...
    <template v-else>
      ...
      <template v-else>
        <div class="legal-header">
          <h1 class="legal-title">{{ cookiePolicy?.title || 'Cookie Policy' }}</h1>
          <p class="legal-updated">Last Updated: {{ cookiePolicy?.lastUpdated || 'January 31, 2026' }}</p>
        </div>
        
        <div class="legal-content">
          <div 
            v-for="section in cookiePolicy?.sections" 
            :key="section.id"
            class="legal-section"
          >
...
const cookiePolicy = ref<{ title: string; lastUpdated: string; sections: Array<{ id: string; heading: string; content: string }> } | null>(null)
...
const loadContent = async () => {
  ...
  try {
    ...
    cookiePolicy.value = viewController.getCookiePolicy()
