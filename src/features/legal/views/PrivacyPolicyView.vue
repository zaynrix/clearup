    ...
    <template v-else>
      ...
      <template v-else>
        <div class="legal-header">
          <h1 class="legal-title">{{ privacyPolicy?.title || 'Privacy Policy' }}</h1>
          <p class="legal-updated">Last Updated: {{ privacyPolicy?.lastUpdated || 'January 31, 2026' }}</p>
        </div>
...
const loadContent = async () => {
  ...
  try {
    ...
    privacyPolicy.value = viewController.getPrivacyPolicy()
