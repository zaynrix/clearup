...
import { homeContentController } from '@/layers/business/controllers/HomeContentController'
import { userController } from '@/layers/business/controllers/UserController'
import { roleController } from '@/layers/business/controllers/RoleController'
import { activityLogController } from '@/layers/business/controllers/ActivityLogController'
...
import type { HomeContent } from '@/layers/business/services/HomeContentService'
...
const loadContent = async () => {
  ...
  try {
    ...
    if (user.value?.id) {
      const adminCheck = await userController.isAdmin(user.value.id)
      isAdmin.value = adminCheck.success && adminCheck.data === true
    }
...
const updateUserRole = async (userId: string, newRole: string) => {
  ...
  isAdminLoading.value = true
  try {
    const result = await userController.updateUserRole(userId, newRole, user.value.id)
    if (result.success) {
      await loadAdminData()
...
const deleteUser = async (userId: string) => {
  ...
  isAdminLoading.value = true
  try {
    const result = await userController.deleteUser(userId, user.value.id)
    if (result.success) {
      await loadAdminData()
      // Force Vue to recognize the change
      users.value = [...users.value]
...
const resetUserPassword = async (userEmail: string) => {
  ...
  isAdminLoading.value = true
  try {
    const result = await userController.resetUserPassword(userEmail, user.value.id)
    if (result.success) {
      await loadAdminData()
      if (activeTab.value === 'activity-logs') {
        await refreshActivityLogs()
